...
3.  uv run python pipeline/s2b_build_chain1.py                  -> outputs/…_chain1/
      |   reads  factors_assumed + chain 2's own tables + the collection cells
      |          (catchment polygons, or the first-mile route clusters — PICKUP_CLUSTERS)
      v
...
| [inputs/pick_up_cluster/](inputs/pick_up_cluster/) | a first-mile routing run — `cluster_summary.csv` + `temp_clustered.csv`, one row per van round and one per collection stop. Chain 1 reads it as its collection cells when `PICKUP_CLUSTERS` names the folder |
