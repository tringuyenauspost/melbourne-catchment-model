"""Step 2 of the build — chain 2, the DELIVERY entity, built entirely from observed data.
    ...
    IN    inputs/factors_observed/    the measurement (written by step 1, s1a_export_chain2_factors.py)
          ...
          inputs/melbourne/           temp_clustered.csv, cluster_summary.csv,
                                      the first-mile catchment polygons (WKT CSV)
...
# produce a len(). `n_catchments()` below counts the same polygons out of the WKT CSV that s2b
# reads, so the printed number is unchanged, the build runs anywhere pandas does, and the
# catchments arrive on the platform as ONE file instead of a CSV and a GeoJSON of the same
# thing. The GeoJSON stays in inputs/ as the converter's source.
...
try:
    ...
    # A REDUCTION HANDED IN COUNTS AS A CACHE. On DataStar there is no pickle -- the reduction
    # arrives as a table and is injected with set_paths() -- and without this the guard was off
    # on the platform, silently, which is where a stale factor CSV is most likely to go unseen.
    import s1a_export_chain2_factors as _s1a_mod
    _have_cache = _SCAN_CACHE.exists() or getattr(_s1a_mod, "_PATHS", None) is not None
except Exception as _e:
    _have_cache = False
    log.info(f"  drift guard skipped — scan pipeline not importable ({_e})")
if not _have_cache:
    log.info(f"  drift guard NOT RUN — no reduction to re-derive from (no {_SCAN_CACHE.name}, "
             f"none injected). The factor CSVs are taken on trust: if a dial changed since they "
             f"were exported, nothing here will notice.")
if _have_cache:
...
def n_catchments(path):
    """Row count of the catchment polygons — what len(gpd.read_file(geojson)) returned.

    One column, so the 19 MB of WKT is not parsed into memory to produce a number that is
    printed once. utilities/convert_catchment_geojson.py writes the file.
    """
    return len(pd.read_csv(path, usecols=["facility_name"]))
N_CATCHMENTS = n_catchments(RAW / "first_mile_catchment_polygons.csv")
