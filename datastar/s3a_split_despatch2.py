...
def main(argv=None):
    ...
    if args.restore:
        ...
        for t in TABLES:
            shutil.copyfile(os.path.join(SAVE, f"{t}.csv"), os.path.join(MODEL, f"{t}.csv"))
        shutil.rmtree(SAVE)
    ...
    for t in TABLES:
        # copyfile, NOT copy: shutil.copy also copies the permission bits, which means a chmod
        # on the destination, and a mounted share can refuse that (`Operation not permitted` on
        # DataStar's /projects/My Files). The undo copy only needs the bytes.
        shutil.copyfile(os.path.join(MODEL, f"{t}.csv"), os.path.join(SAVE, f"{t}.csv"))
