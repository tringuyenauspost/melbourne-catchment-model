...
╔═════════════════════════════════════════════════════════════════════════════════════╗
║ STANDALONE COPY of pipeline/s1a_export_chain2_factors.py, for Optilogic DataStar.   ║
║ It imports nothing from pipeline/. Taken 2026-09-18 and verified to reproduce that  ║
║ build exactly: the reduction matches consignment_paths.pkl on all 56 columns and    ║
║ 161,976 consignments, and every obs_*.csv comes out byte-identical.                 ║
║                                                                                     ║
║ FIVE deliberate differences, and nothing else was touched:                          ║
║   1. _paths/_log/_report imports -> $MELB_DATA_ROOT, stdlib logging, no report card. ║
║   2. lodgement_geography reads CSVs via pandas + shapely, not .geojson via           ║
║      geopandas (DataStar accepts neither). Places all 2,569 facilities identically.  ║
║   3. set_scans() feeds it a DataFrame; _as_text restores the dtype=str semantics     ║
║      a CSV read gave it, which this file depends on.                                 ║
║   4. set_paths() lets PART 2 run on a published reduction instead of the pickle.      ║
║   5. pack_paths/unpack_paths carry the four LIST columns through a SQL table.        ║
║                                                                                     ║
║ IT IS A FORK. A change to s1a does not reach it. Re-take it, do not hand-edit it.    ║
╚═════════════════════════════════════════════════════════════════════════════════════╝
...
# ── WHERE THE DATA IS ────────────────────────────────────────────────────────────────
# `pipeline/_paths.py` resolved these by walking up from the repo. Here they are one env
# var, because on DataStar there is no repo to walk.
DATA_ROOT = Path(os.environ.get("MELB_DATA_ROOT", "/projects/My Files/DataStar"))


def _inputs_dir(root):
    """DataStar's upload folder is `Raw Inputs`; the repo checkout calls it `inputs`.

    Whichever exists wins, so the same file runs on the platform and against a local
    checkout without editing a path. `MELB_INPUTS` overrides both.
    """
...
RAW = INPUTS / "melbourne"                      # the raw extract and the boundary CSVs
FASS = INPUTS / "factors_assumed"               # sites.csv, dials.csv
# WRITTEN, not read. If `Raw Inputs` is read-only on the platform, point MELB_FOBS at a
# writable folder -- the obs_*.csv here are an intermediate; the tables are the deliverable.
FOBS = Path(os.environ.get("MELB_FOBS") or (INPUTS / "factors_observed"))
OUT = DATA_ROOT / "outputs/melbourne_scan_path_analysis"

log = logging.getLogger("melb.scan_reduction")
...
METRO_BOUNDARY = RAW / "first_mile_catchment.csv"      # one WKT row, EPSG:4326
ROUTE_STOPS = RAW / "first_mile_route_stops.csv"       # 10,606 first-mile pickup points
...
def lodgement_geography(lodge_fac):
    ...
    Same three layers as the original -- model node, then first-mile pickup point, then the
    suburb on a pickup point's address -- but it reads CSVs through pandas + shapely instead
    of .geojson through geopandas, because DataStar takes neither geopandas nor .geojson.
    The CSVs come from utilities/convert_first_mile_geo.py and were verified to place all
    2,569 lodgement facilities identically.

    Original note: docs/export_chain2_factors.md#lodgement-geography
    """
    ...
    def node_of(name):
        for cand in (LODGE_NODE_ALIAS.get(name), SITE_NODE.get(SORT_SITE.get(name)),
                     pud_node.get(LODGE_PUD.get(name)), name):
            if cand and cand.upper() in points:
                return points[cand.upper()]
        return None

    # layer 2 — the first-mile pickup points, by name
    stops = pd.read_csv(ROUTE_STOPS)
    ...
    boundary = shapely.from_wkt(pd.read_csv(METRO_BOUNDARY)["geometry"].iloc[0])
    shapely.prepare(boundary)
    ok = place.lon.notna()
    place["metro"] = pd.Series(pd.NA, index=place.index, dtype="object")
    place.loc[ok, "metro"] = shapely.contains_xy(
        boundary, place.lon[ok].to_numpy(float), place.lat[ok].to_numpy(float))
...
def print_lodge_geography(s, w, unit):
    ...
    # `lodge_metro` is object dtype holding True/False/NA. A bare .fillna().astype(bool)
    # is a silent downcast pandas 2 warns about and a future pandas will change -- and it
    # decides METRO vs REGION, so it is made explicit rather than left to a default.
    _metro = s.lodge_metro.astype("boolean")
    inside = vic & _metro.fillna(False).astype(bool)
    outside = vic & s.lodge_metro.notna() & ~_metro.fillna(True).astype(bool)
...
SCAN_COLS = ["Consignment_ID", "Event_seq", "Event_type", "Event_facility_name",
             "Terminating_facility_name", "Product_type", "Article_count",
             "STE_NAME21", "Event_date", "Event_local_time"]
_SCANS = None      # set by set_scans(); read_scans() returns it instead of reading SCAN
_PATHS = None      # set by set_paths(); load_paths() returns it instead of reducing again


def _as_text(s):
    """Column -> what read_csv(dtype=str) would have produced: text, nulls preserved.

    A SQL read hands back native types -- Event_seq and Article_count as integers, and
    Event_date/Event_local_time possibly as real DATE/TIME values. This file CONCATENATES
    those two into a timestamp and carries Article_count as text until consignment_identity()
    calls to_numeric on it, so anything else silently changes the answer.
    """
    if pd.api.types.is_datetime64_any_dtype(s):
        out = s.dt.strftime("%Y-%m-%d %H:%M:%S").str.replace(" 00:00:00", "", regex=False)
    elif pd.api.types.is_timedelta64_dtype(s):
        out = s.astype(str).str.extract(r"(\d{2}:\d{2}:\d{2})", expand=False)
    elif pd.api.types.is_float_dtype(s):
        out = s.map(lambda v: "" if pd.isna(v) else f"{v:.0f}" if float(v).is_integer() else str(v))
        out = out.replace("", np.nan)
    else:
        out = s.astype(object)
        out = out.where(s.isna(), out.astype(str))
    return out.where(s.notna(), np.nan)


def set_scans(frame):
    """Feed the reduction a scan table instead of a CSV path."""
    global _SCANS
    missing = [c for c in SCAN_COLS if c not in frame.columns]
    if missing:
        raise SystemExit(f"scan table is missing {missing}; got {list(frame.columns)}")
    df = pd.DataFrame({c: _as_text(frame[c]) for c in SCAN_COLS})
    df["dt"] = pd.to_datetime(df.Event_date + " " + df.Event_local_time.fillna("00:00:00"),
                              errors="coerce")
    df["Event_seq"] = pd.to_numeric(df["Event_seq"])
    _SCANS = df.sort_values(["Consignment_ID", "Event_seq"])
    return _SCANS


# Four reduction columns hold LISTS. A SQL table cannot, and a round-trip through one turns
# them into the string "['MGF', 'TPF']" -- which PART 2 then indexes CHARACTER by character,
# in silence, because s[0] on a string is still a string.
LIST_COLS = ("full_chain", "states_touched", "path_sites", "chain")
LIST_SEP = " > "        # the separator `path` and `full_path` already use


def pack_paths(p):
    """Reduction frame -> a frame a table can hold."""
    out = p.copy()
    for c in LIST_COLS:
        if c in out.columns:
            out[c] = out[c].map(lambda v: LIST_SEP.join(v) if isinstance(v, (list, tuple)) else v)
    return out


def _canon_columns(df):
    """Undo Starburst's lowercasing: map columns back onto PATH_COLS' exact spelling."""
    want = {c.lower(): c for c in PATH_COLS}
    return df.rename(columns={c: want.get(str(c).lower(), c) for c in df.columns})


def unpack_paths(df):
    """A published paths table -> the reduction frame PART 2 expects."""
    out = _canon_columns(df).copy()
    for c in LIST_COLS:
        if c in out.columns:
            out[c] = out[c].map(
                lambda v: [] if v is None or (not isinstance(v, str) and pd.isna(v)) or v == ""
                else (list(v) if isinstance(v, (list, tuple)) else v.split(LIST_SEP)))
    return out


def set_paths(frame):
    """Feed PART 2 a reduction that has already been built and published."""
    global _PATHS
    _PATHS = unpack_paths(frame)
    return _PATHS
...
def read_scans():
    ...
    if _SCANS is not None:
        return _SCANS
    df = pd.read_csv(SCAN, dtype=str, usecols=SCAN_COLS)
    df["dt"] = pd.to_datetime(df.Event_date + " " + df.Event_local_time.fillna("00:00:00"),
                              errors="coerce")
    df["Event_seq"] = pd.to_numeric(df["Event_seq"])
...
def source(df, fc, index, articles=None):
    ...
    s["lodge_band"] = np.where(
        ...
        np.where(placed & s.lodge_metro.astype("boolean").fillna(False).astype(bool), "METRO",
...
def main(argv=None, rebuild=False):
    """Export the observed factors.

    No argparse. It existed only to offer --rebuild, and reading `__doc__` for the parser
    description crashes outright if this file is pasted somewhere without its module
    docstring. `argv` is still accepted so callers passing a list keep working, and
    `rebuild` is inert once set_paths() has injected a reduction.
    """
    if argv:
        rebuild = rebuild or "--rebuild" in argv

    dials = read_dials()
    p = load_paths(rebuild=rebuild).set_index("Consignment_ID")
    ...
    # The phase report card is DROPPED in this copy. It lived in pipeline/_report.py, it is
    # commentary rather than output, and every factor CSV above is written without it.
